...
void erase(int x) {                   
  ...
  splay(root,v); // v now is at the top
...
bool find(int x) {  
  ...
  Vertex* v = find(root,x); // returns vertex that contains the key
  splay(root,v); // after every operation in a splay tree
...
long long sum(int from, int to) {
  ...
  // we must update the sums of all the vertices here
  update(left);
  update(middle);
  update(right);
